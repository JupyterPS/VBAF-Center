...
function Invoke-VBAFCenterWriteBack {
    ...
    if (Test-Path $logFile) {
        try { $raw = Get-Content $logFile -Raw | ConvertFrom-Json; $existing = @($raw | ForEach-Object { #Requires -Version 5.1
<#
.SYNOPSIS
    VBAF-Center Phase 18 — Write-back
.DESCRIPTION
...
function Undo-VBAFCenterWriteBack {
    ...
    if ($entry.RollbackExpiry -ne "") {
        $expiry = [DateTime]::ParseExact(([string]($entry.RollbackExpiry | Select-Object -First 1)).Trim(), "yyyy-MM-dd HH:mm:ss", $null)
    ...
    Write-Host ""
    Write-Host ("Rollback: {0} — {1}" -f $CustomerID, ([string]$entry.ActionName)) -ForegroundColor Yellow
    Write-Host ("  Original action : {0} at {1}" -f ([string]$entry.ActionName), ([string]$entry.Timestamp)) -ForegroundColor White
    ...
    if ($entry.PSObject.Properties["RollbackAvailable"]) { $entry.RollbackAvailable = $false }
